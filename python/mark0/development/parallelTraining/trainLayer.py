...
# Update the mask which allows to select only the elements that have already been active.
#
# This can be used to mask the LTD, applying it only on those neurons that have already
# generated an output spike, and the LTP, applying it only on those inputs that have
# already received a spike. This is useful at the beginning of the training to avoid the
# modification of synapses still not involved in any spike propagation.
#
# INPUT PARAMETERS:
#
# 	1) layerDict: dictionary containing the parameters of all the neurons inside the
# 	layer.
#
# 	2) maskName: string containing the mask to modify. This must correspond to one of
# 	the two mask names used as keys of layerDict.
#
# 	3) spikeArray: array of spikes used to update the mask. The function has been
# 	developed to update the output mask and the input mask, so the array of spike can
# 	be one between the input and the output spike array.
#
# The function simply puts in or the values already stored into the mask and the values
# belonging to the array of spikes. In this way the elements that have already been active
# remain active, while the elements that ae active for the first time in the current cycle
...
# 	and the exponential time constant. Whichever ratio Delta_t/tau can be expressed
# 	as a multiple of this quantity. Delta_t in this case represents the difference
# 	between when the inputs have generated an event and when the neuron has fired 
# 	for the last time
#
