...
def searchMaxAndUppdate(labelCounters, couterIndexs, label, neuronsAssigned,
	...
	INPUT:
		...
		1) labelCounters: NumPy array containing the total amount of 
		spikes generated by each neuron for the current label.

		2) counterIndexes: indexes of the neurons that have generated
		the maximum amount of spikes for the current label.

		3) label: current label.

		4) neuronsAssigned: NumPy array of boolean values. One value for
		each neuron.

		5) labelsAssigned: NumPy array of boolean values. One value for
		each label.

		6) assignments: NumPy array containing the assigned labels. One
	...
	if maxFound ==1:
		...
		maxIndex = counterIndexes[maxIndex]
...
labelsAssigned = np.zeros(10).astype(bool)

searchMaxAndUppdate(labelCounters, couterIndexs, label, neuronsAssigned,
		labelsAssigned, assignments)
