...
def singleImageTraining(trainDuration, restTime, dt, image, network,
	...
	inputCounter = np.sum(spikesTrains, axis = 0)

	# Train the network with the spikes sequences associated to the pixels.
	inputIntensity, currentIndex, accuracies, spikesCounter = \
		train(
			network, 
			networkList, 
			spikesTrains, 
	...
	return inputIntensity, currentIndex, accuracies, spikesCounter,\
		inputCounter
...
def train(network, networkList, spikesTrains, dt_tauDict, stdpDict,
	...
	if np.sum(spikesCounter[-1]) < countThreshold:
