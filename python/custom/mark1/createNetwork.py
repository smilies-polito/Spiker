...
def intraLayersSynapses(network, synapseName, mode, networkList, weightFile,
	...
	network[synapseName] = {
		...
		# Initialize the instants corresponding to the input spikes
		"t_in"		: np.zeros((1, networkList[layer - 1])),

		# Initialize the instants corresponding to the output spikes
...
def initializeWeights(mode, networkList, weightFile, layer, scaleFactor):
	...
	if mode == "train":
		...
		return (np.random.rand(networkList[layer], networkList[layer - 1])
			+ 0.01)*scaleFactor
