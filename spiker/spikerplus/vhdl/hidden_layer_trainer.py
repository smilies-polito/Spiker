...
class HiddenLayerTrainer(VHDLblock):
	...
	def vhdl(self, debug=False):
		...
		# Vivado waveform-debug signals, mirrored verbatim from the
		# hand-coded reference. Functionally dead: they only exist so
		# individual weights can be monitored in a waveform viewer.
		self.architecture.customTypes.add(
		...
		#       -> weight += NEURON_CONSTANTS(i)
		debug_block = (
			"debug_assign_gen: for i in 0 to n_hidden_neurons-1 generate\n"
			"    begin\n"
		...
		self.architecture.bodyCodeHeader.add(debug_block)
