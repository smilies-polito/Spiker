...
def imgToSpikeTrain(image, num_steps, bitWidth, taps, seed):
	...
	INPUT:
		...
		3) num_steps: total amount of time steps associated to the
	...
	random2D = randomArrayGen(num_steps, 1, bitWidth, taps,
			seed)

	# Convert the image into spikes trains
	return poisson(image, random2D, bitWidth)





def poisson(image, random2D, bitWidth):
