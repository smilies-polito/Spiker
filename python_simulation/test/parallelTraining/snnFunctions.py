...
# Simulate the training of a network of spiking neurons for the desired number of 
# simulation cycles.
#
# INPUT PARAMETERS:
#
...
def createTrainNetworkDictList(v_th_list, v_reset, w_min_list, w_max_list, networkList):
	return [createLayerDict(v_th_list[i], v_reset, w_min_list[i], w_max_list[i], 
		networkList[i+1], networkList[i]) for i in range(len(networkList)-1)]
...
def plotTestNetworkResults(networkList, inEvents_evolution, outEventsEvol_list, 
